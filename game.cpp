...
{ // reads the map text file and stores all the characters in an array
...
void Game::blit()
{ // goes through the array and displays the map
	screen = SDL_GetWindowSurface(gWindow);
	SDL_Surface *wallImage = NULL;
	SDL_Surface *foodImage = NULL;
	SDL_Surface *pacmanImage = NULL;
	wallImage = SDL_LoadBMP("./assets/wall.bmp");
	foodImage = SDL_LoadBMP("./assets/point.bmp");
	pacmanImage = SDL_LoadBMP("./assets/pacman.png");
	if (pacmanImage == NULL)
	{
...
bool Game::loadMedia()
{
	...
	gTexture = loadTexture("./assets/pac_back.png");
...
void Game::run()
{
	...
	while (!quit)
	{
		if (pacmania.p->lives == 0)
		{
			...
			gTexture = loadTexture("./assets/gmeover.png");
		}
		else if (pacmania.p->isRunning == false)
		{
			// for(Ghost* g: pacmania.ghosts)delete g;
			gTexture = loadTexture("./assets/gamewin.png");
		}
		...
		while (SDL_PollEvent(&e) != 0)
		{
			...
			if (e.type == SDL_MOUSEBUTTONDOWN)
			{
				...
				if (xMouse > 40 && xMouse < 315 && yMouse > 250 && yMouse < 370)
					blit();
			}
		...
		if (blit_called == false)
			pacmania.drawObjects();
		else if (pacmania.p->lives > 0 && pacmania.p->isRunning == true)
		{
			...
			case 'r':
				...
				{
					SDL_Surface *wallImage = NULL;
					wallImage = IMG_Load("./assets/black.png");
